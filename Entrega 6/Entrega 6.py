...
names = ['AinvB.txt','AinvB_npSolve.txt','AinvB_spSolve.txt','AinvB_spSolve_symmetric.txt','AinvB_spSolve_pos.txt','AinvB_spSolve_pos_overwrite.txt']
...
for N in Ns:
    ...
    for i in range(ciclos):
        ...
        B = np.ones(N)
        t1 = perf_counter()
        AinvB = sp.linalg.solve(A, B, assume_a='sym')
        t2 = perf_counter()
        dt = t2-t1
        dts[i][3]= dt
        ...
        B = np.ones(N)
        t1 = perf_counter()
        AinvB = sp.linalg.solve(A, B, assume_a='pos')
        t2 = perf_counter()
        dt = t2-t1
        dts[i][4]= dt
        ...
        B = np.ones(N)
        t1 = perf_counter()
        AinvB = sp.linalg.solve(A, B, assume_a='pos',overwrite_a=True)
        t2 = perf_counter()
        dt = t2-t1
        dts[i][5]= dt
